...
if __name__ == "__main__":
	## Check the no. of command line arguments
	if len(sys.argv) != 3:
		print("Please ensure query and required no. of documents are provided.")
		sys.exit(1)
	else:
		...
		## Check if documents provided are sufficient
		if k > no_of_docs:
			print("More documents are to be provided.")
			sys.exit(1)
		else:
			...
			doc_scores = top_docs.take(k)
			print("Top-k relevant documents: " + str(doc_scores))
			output_file = open('output.txt', 'w')
			for doc_score in doc_scores:
				output_file.write(' '.join(str(s) for s in doc_score) + '\n')
			output_file.close()
